...
plotter <- function(df, df_title){
  ggplot(df, aes(ele, timeElapsed, color = timeElapsed)) +
    ...
    labs(subtitle = "Time vs Size",
       y = "Number of Elementary Operations (Averaged)",
...
comb_plotter <- function(df, df_title){
  ...
  theme_minimal() +
  labs(subtitle = "Time vs Size",
       y = "Number of Elementary Operations",
       x = "Number of Elements",
       title = df_title) +
  stat_poly_eq(parse=T, aes(label = ..eq.label..), formula=y~x) 
}
...
mergeSort <- function(vec){
  ...
  if(n <= 1) return (list("vec" = vec, "operations" = op + 1))
    ...
    op <- op + left_list$operations + right_list$operations + res$operations + 4
...
quickSort <- function(vec){
  ...
  if(length(vec) > 1){
    pivot <- vec[1]
    left_list <- quickSort(vec[vec < pivot])
    right_list <- quickSort(vec[vec > pivot])
    op <- op + 4 + length(vec)
    vec <- c(left_list$vec,vec[vec == pivot], right_list$vec)
    op <- op + left_list$operations + right_list$operations + 2 + length(vec)
    return (list("vec" = vec, "operations" = op))
  }else{
    return (list("vec" = vec, "operations" = op + 1))
  }
}
```
